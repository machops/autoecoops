...
import { startDriftDetection, stopDriftDetection } from './reconcilers/argocd.reconciler';
...
async function bootstrap(): Promise<void> {
  logger.info({ port: config.PORT, env: config.NODE_ENV }, 'Starting infra-manager');
  startDriftDetection();

  const server = app.listen(config.PORT, () => { logger.info({ port: config.PORT }, 'infra-manager listening'); });

  const shutdown = async (signal: string) => {
    logger.info({ signal }, 'Shutting down infra-manager');
    stopDriftDetection();
...
bootstrap().catch((err) => { logger.fatal({ err }, 'Failed to start infra-manager'); process.exit(1); });
